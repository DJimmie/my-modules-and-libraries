...
class SqliteDB():
    ...
    def database_server(self):
        """This function is not in use as of 10-26-2020"""
        server=self.server
        return server
    ...
    def update_record (self,the_sql_command,values):
    ...
    def query_list(self,the_sql_command):
        """Query the database for combobox drop down values list"""
        
        print (the_sql_command)

        try:
            query=self.query_the_database(the_sql_command) 
    ...
    def query_the_database(self,the_sql_command):
    ...
    def executeTheCommand(self,the_sql_command,values=None):
